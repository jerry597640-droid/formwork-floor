...
- 新增組別自動帶入目前第一組的材料尺寸（含跨度）、材料參數（彈性係數、容許應力、撓度限值）、載重條件（樓板厚度、其他靜載重、活載重、衝擊載重）、各構件梁分析模式及支撐型式與參數；各組均可獨立修改。第一組後續調整只影響之後新增的組別，不回寫已建立的組別。「新增組別」一律以當下第一組為來源；若需沿用目前正在編輯組別的全部條件，請使用「複製本組」。
